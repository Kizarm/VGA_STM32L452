...
class data_endpoint : public usbd_endpoint {
  MsClass * ifc;
  public:
    data_endpoint (usbd_device & d, MsClass * i) : usbd_endpoint(d), ifc(i) {};
    void evt_handler (usbd_events event, uint8_t ep);
};

/// @brief Vlastní třída pro Mass Storage s podporou SCSI.
class MsClass : public usbd_interface {
  data_endpoint de;
  public:
    /// Konstruktor
    MsClass(usbd_device & p);
