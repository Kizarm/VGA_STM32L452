...
void ChipPIO8255::ChipReset (bool clearNotifyFunc) {
  if (clearNotifyFunc == true) ClearAllNotifyFunctions();
...
void ChipPIO8255::CpuWrite (TPIOPort dest, BYTE val) {
  ...
  case PP_CWR :
    if (val & CWR_MASK) {
      // setting of PIO mode
      CWR = val;
      if ( (CWR & GA_MODE) == GA_MODE)      CWR &= ~GA_MODE1;
      ...
      if ( (CWR & GB_MODE) != GB_MODE0)     InLatchB = 0;
      OutLatchB = 0;

      InBufferC = 0;
      if ( (CWR & GB_MODE) == GB_MODE0)     InBufferC |= 0x07; // pull-up
      if ( (CWR & GA_MODE) == GA_MODE0)     InBufferC |= 0xF8;

//        oldVal = OutLatchC;
      OutLatchC = 0;
      if ( (CWR & GB_MODE) != GB_MODE0)     OutLatchC |= _OBFB_MASK;
      if ( (CWR & GA_MODE) != GA_MODE0)     OutLatchC |= _OBFA_MASK;
...
BYTE ChipPIO8255::CpuRead (TPIOPort src) {
  ...
  case PP_PortA :
    ...
    if (mode == (GA_MODE0 | PORTA_INP)) OnCpuReadA();
  ...
  case PP_PortB :
    ...
    // notification is enabled only in mode 0, input,
    // in other modes there are handshake signals
    if (mode == (GB_MODE0 | PORTB_INP))  OnCpuReadB();

    // mode 0, output
    if (mode == (GB_MODE0 | PORTB_OUT))      ret_val = OutLatchB; // reading of value from output latch
    // mode 0, input
    else if (mode == (GB_MODE0 | PORTB_INP)) ret_val = InBufferB; // reading of value from input
    // mode 1, output
    else if (mode == (GB_MODE1 | PORTB_OUT)) ret_val = OutLatchB; // reading of value from output latch
    // mode 1, input
...
/*
    if (OnCpuReadCH.isset() || OnCpuReadCL.isset()) {
      // mode 0, input
      if ( (CWR & (GA_MODE | PORTCH_DIR)) == (GA_MODE0 | PORTCH_INP))  OnCpuReadCH();
      if ( (CWR & (GB_MODE | PORTCL_DIR)) == (GB_MODE0 | PORTCL_INP))  OnCpuReadCL();
    } else if (OnCpuReadC.isset()) {
      // mode 0, input
      if ( (CWR & (GB_MODE | PORTCL_DIR)) == (GB_MODE0 | PORTCL_INP)
           || (CWR & (GA_MODE | PORTCH_DIR)) == (GA_MODE0 | PORTCH_INP)) OnCpuReadC();
    }
*/
    // mode 0, output
    if ( (CWR & (GB_MODE | PORTCL_DIR)) == (GB_MODE0 | PORTCL_OUT))
