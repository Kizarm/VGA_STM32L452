...
static inline void ExtInit () {
  ...
  EXTI.IMR1. B.MR5 = SET;
  EXTI.RTSR1.B.TR5 = SET;
  NVIC_EnableIRQ  (EXTI9_5_IRQn);
  
}
Keyboard::Keyboard() : ring(), sclk(GpioPortA, 5, GPIO_Mode_IN), miso(GpioPortA, 6, GPIO_Mode_IN) {
...
void Keyboard::irq() {
  ...
  if (bitcount++ == 10)  {
    // budeme vyhodnocovat v main loop, není to úplně krátké, v přerušení stejně ruší.
    ring.Write (data);
    bitcount = 0;
    data = 0x00;
  }
